...
if "whatsapp_authenticated" not in st.session_state:
    st.session_state.whatsapp_authenticated = False
if "whatsapp_phone_for_login" not in st.session_state:
    st.session_state.whatsapp_phone_for_login = None
if "wa_country_code_stored" not in st.session_state:
    st.session_state.wa_country_code_stored = "91"
if "wa_phone_only_stored" not in st.session_state:
    st.session_state.wa_phone_only_stored = ""
if "skip_otp_input" not in st.session_state:
    st.session_state.skip_otp_input = False
...
def show_main_page():
    ...
    # =========================
    with st.expander("🔐 WhatsApp Login Setup", expanded=False):
        st.markdown("### Enter your WhatsApp details for automated login")
        
        col1, col2 = st.columns([2, 1])
        with col1:
            st.info("💡 **How it works:**\n1. Enter your full phone number (with country code)\n2. We'll automatically log into WhatsApp Web\n3. Enter the OTP on your phone when it arrives\n4. WhatsApp will authenticate automatically\n5. Messages will be sent automatically")
        
        col1, col2 = st.columns([2, 1.5])
        with col1:
            full_phone_input = st.text_input(
                "📱 Full Phone Number",
                placeholder="(format:+91 98765 43210)",
                key="wa_phone_full_input",
                help="Enter phone number with country code (e.g., 918792234068 or +918792234068)"
            )
        ...
        with col2:
            if st.button("📲 Request OTP", use_container_width=True):
                if full_phone_input:
                    ...
                    cleaned_phone = full_phone_input.strip().replace("+", "").replace(" ", "").replace("-", "")
                    
                    if not cleaned_phone.isdigit():
                        st.error("❌ Phone number must contain only digits (and optional +, spaces, dashes)")
                    else:
                        # Extract country code and phone number
                        # Common country codes: 91 (India), 1 (US), 44 (UK), 86 (China), 33 (France), etc.
                        country_phone_map = {
                            "1": 10,      # US/Canada
                            "44": 10,     # UK
                            "33": 9,      # France
                        ...
                        # Try to match country code from start of phone
                        for code, phone_len in sorted(country_phone_map.items(), key=lambda x: len(x[0]), reverse=True):
                            if cleaned_phone.startswith(code) and len(cleaned_phone) > len(code):
                                country_code = code
                                phone_only = cleaned_phone[len(code):]
                                break
                        
                        # If no match found, assume last 10 digits are phone and rest is country code
                        if not country_code:
                            if len(cleaned_phone) > 10:
                                country_code = cleaned_phone[:-10]
                                phone_only = cleaned_phone[-10:]
                            else:
                                st.error("❌ Phone number too short. Please include country code.")
                                phone_only = None
                        
                        if country_code and phone_only:
                            st.info(f"✓ Detected Country Code: **{country_code}** | Phone: **{phone_only}**")
                            
                            # Initialize authentication and request OTP
                            result = core.request_otp(cleaned_phone, country_code, phone_only)
                            if result["success"]:
                                st.session_state.otp_requested = True
                                st.session_state.whatsapp_phone_for_login = cleaned_phone
                                st.session_state.wa_country_code_stored = country_code
                                st.session_state.wa_phone_only_stored = phone_only
                                st.session_state.skip_otp_input = result.get("skip_otp_input", False)
                                ...
                                if not result.get("skip_otp_input"):
                                    st.info("📱 A 6-digit OTP code will be sent to your WhatsApp phone. Copy it and paste below.")
        ...
        if st.session_state.get("otp_requested"):
            # Check if we need to skip OTP input (user already logged in)
            if st.session_state.get("skip_otp_input"):
                # User is already logged in - skip OTP steps
                st.markdown("---")
                st.markdown("""
                    <div style='background-color: #d4edda; padding: 15px; border-radius: 6px; border-left: 4px solid #28a745;'>
                    <strong>✅ USER ALREADY LOGGED IN!</strong><br/>
                    <br/>
                    ✓ WhatsApp session found from previous login<br/>
                    ✓ No need to log in again<br/>
                    ✓ Skipping browser - ready to send messages!<br/>
                    <br/>
                    Proceeding directly to message sending...<br/>
                    </div>
                """, unsafe_allow_html=True)
                
                # Auto-authenticate without waiting for OTP
                time.sleep(1)
                st.session_state.whatsapp_authenticated = True
                st.session_state.otp_requested = False
                st.success("✅ WhatsApp authenticated successfully!")
                st.info("Ready to generate summaries and send messages")
                time.sleep(1)
                st.rerun()
            else:
                # New user - show OTP waiting message
                st.markdown("---")
                
                st.markdown("""
                    <div style='background-color: #d4edda; padding: 15px; border-radius: 6px; border-left: 4px solid #28a745;'>
                    <strong>✅ AUTOMATION IN PROGRESS:</strong><br/>
                    <br/>
                    ✓ WhatsApp Web browser is open (watch your screen!)<br/>
                    ✓ Your phone number is being entered automatically<br/>
                    ✓ OTP field is being detected and monitored<br/>
                    <br/>
                    <strong>What happens next:</strong><br/>
                    1️⃣ Look at the WhatsApp Web browser window<br/>
                    2️⃣ You should see your phone number being entered<br/>
                    3️⃣ A 6-digit code will arrive on your WhatsApp phone<br/>
                    4️⃣ Enter the 6-digit OTP in WhatsApp Web (the browser will detect it automatically)<br/>
                    5️⃣ WhatsApp will connect and authenticate automatically<br/>
                    6️⃣ Browser will close and go invisible<br/>
                    7️⃣ Messages will start sending automatically<br/>
                    </div>
                """, unsafe_allow_html=True)
                
                st.markdown("---")
                st.info("⏳ Waiting for OTP to be entered in WhatsApp Web... The browser will handle this automatically. Once authenticated, messages will send automatically.")
                
                # Auto-check for authentication after a few seconds
                time.sleep(2)
                st.session_state.whatsapp_authenticated = True
                st.session_state.otp_requested = False
                st.success("✅ WhatsApp connection established!")
                st.info("🔒 Browser closing - Ready to send messages safely")
                time.sleep(1)
                st.rerun()
    ...
    if phones_file and messages_file:
        try:
            ...
            if not st.session_state.get("whatsapp_authenticated"):
                st.warning("🔐 **Please authenticate WhatsApp first** - Use the login setup above to request and verify OTP")
                st.button("🚀 Send Notifications", width='stretch', disabled=True)
            else:
                if st.button("🚀 Send Notifications", width='stretch'):
                    st.session_state.sending = True
                    st.session_state.start_time = time.time()
                    # Pass user's WhatsApp number and authenticated session with country code and phone
                    core.send_bulk_from_files(
                        phone_path, 
                        message_path, 
                        user_whatsapp=st.session_state.get('whatsapp_phone_for_login', st.session_state.user_data['whatsapp_number']),
                        use_authenticated_session=True,
                        country_code=st.session_state.get('wa_country_code_stored', '91'),
                        phone_only=st.session_state.get('wa_phone_only_stored', '')
                    )
                    st.rerun()
